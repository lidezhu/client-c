...
public:
  Oracle(ClientPtr pd_client_, std::chrono::milliseconds update_interval_)
      : pd_client(pd_client_), update_interval(update_interval_),
  ...
  int64_t untilExpired(uint64_t lock_ts, uint64_t ttl) {
    return extractPhysical(lock_ts) + ttl - extractPhysical(last_ts);
  }
