...
uint64_t sendTxnHeartBeat(Backoffer &bo, Cluster *cluster,
                          std::string &primary_key, uint64_t start_ts,
                          uint64_t ttl);
...
struct TTLManager {
private:
  enum TTLManagerState { StateUninitialized = 0, StateRunning, StateClosed };
...
public:
  ...
  void run(TwoPhaseCommitter &committer) {
    ...
    if (!state.compare_exchange_strong(expected, StateRunning,
  ...
  void close() {
    ...
    state.compare_exchange_strong(expected, StateClosed,
...
private:
  enum Action { ActionPrewrite = 0, ActionCommit, ActionCleanUp };

  struct BatchKeys {
    RegionVerID region;
    std::vector<std::string> keys;
    BatchKeys(const RegionVerID &region_, const std::vector<std::string> &keys_)
        : region(region_), keys(keys_) {}
  };
...
public:
  ...
  void prewriteKeys(Backoffer &bo, const std::vector<std::string> &keys) {
    committer.prewriteKeys(bo, keys);
  }

  void commitKeys(Backoffer &bo, const std::vector<std::string> &keys) {
    committer.commitKeys(bo, keys);
  }
