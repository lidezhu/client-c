...
enum Jitter { NoJitter = 1, FullJitter, EqualJitter, DecorrJitter };
...
struct Backoff {
  ...
  Backoff(int base_, int cap_, Jitter jitter_)
      : base(base_), cap(cap_), jitter(jitter_), attempts(0) {
    if (base < 2) {
      base = 2;
    }
    last_sleep = base;
  ...
  int sleep(int max_sleep_time) {
    ...
    case EqualJitter:
      ...
      break;
    case DecorrJitter:
      sleep_time = int(std::min(
          double(cap), double(base + rand() % (last_sleep * 3 - base))));
    }
...
struct Backoffer {
  ...
  void backoffWithMaxSleep(BackoffType tp, int max_sleep_time,
                           const Exception &exc);
};
