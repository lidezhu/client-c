...
struct Cluster {
  ...
  Cluster(const std::vector<std::string> &pd_addrs,
          const std::string &learner_key, const std::string &learner_value)
      : pd_client(std::make_shared<pd::CodecClient>(pd_addrs)),
        region_cache(std::make_unique<RegionCache>(pd_client, learner_key,
                                                   learner_value)),
        rpc_client(std::make_unique<RpcClient>()),
        oracle(std::make_unique<pd::Oracle>(
            pd_client, std::chrono::milliseconds(oracle_update_interval))),
        lock_resolver(std::make_unique<LockResolver>(this)) {}

  // TODO: When the cluster is closed, we should release all the resources
  // (e.g. background threads) that cluster object holds so as to exit
  // elegantly.
  ~Cluster() {}
