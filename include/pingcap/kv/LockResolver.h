...
// TxnStatus represents a txn's final status. It should be Lock or Commit or
// Rollback.
struct TxnStatus {
  uint64_t ttl;
  uint64_t commit_ts;
...
struct Lock {
  ...
  Lock(const ::kvrpcpb::LockInfo &l)
      : key(l.key()), primary(l.primary_lock()), txn_id(l.lock_version()),
        ttl(l.lock_ttl()), txn_size(l.txn_size()), lock_type(l.lock_type()),
...
inline std::string getLockInfo(LockPtr &lock) {
  return "key: " + lock->key + " primary: " + lock->primary +
         " txn_start_ts: " + std::to_string(lock->txn_id) +
         " lock_for_update_ts: " + std::to_string(lock->lock_for_update_ts) +
         " ttl: " + std::to_string(lock->ttl) +
...
public:
  LockResolver(Cluster *cluster_)
      : cluster(cluster_), log(&Logger::get("pingcap/resolve_lock")) {}
  ...
  //    commit status.
  // 3) Send `ResolveLock` cmd to the lock's region to resolve all locks belong
  // to
  //    the same transaction.

  int64_t ResolveLocks(Backoffer &bo, uint64_t caller_start_ts,
                       std::vector<LockPtr> &locks,
                       std::vector<uint64_t> &pushed);

  int64_t resolveLocks(Backoffer &bo, uint64_t caller_start_ts,
                       std::vector<LockPtr> &locks,
                       std::vector<uint64_t> &pushed, bool for_write);

  int64_t resolveLocksForWrite(Backoffer &bo, uint64_t caller_start_ts,
                               std::vector<LockPtr> &locks);
...
private:
  ...
  void resolveLock(Backoffer &bo, LockPtr lock, TxnStatus &status,
                   std::unordered_set<RegionVerID> &set);

  void resolvePessimisticLock(Backoffer &bo, LockPtr lock,
                              std::unordered_set<RegionVerID> &set);

  TxnStatus getTxnStatusFromLock(Backoffer &bo, LockPtr lock,
                                 uint64_t caller_start_ts);

  TxnStatus getTxnStatus(Backoffer &bo, uint64_t txn_id,
                         const std::string &primary, uint64_t caller_start_ts,
