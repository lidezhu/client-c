...
template <class T> class RpcCall {
...
public:
  RpcCall(std::shared_ptr<T> req_)
      : req(req_), resp(std::make_unique<S>()),
  ...
  void setCtx(RPCContextPtr rpc_ctx) {
    ...
    context->set_allocated_region_epoch(
        new metapb::RegionEpoch(rpc_ctx->meta.region_epoch()));
  ...
  void call(std::shared_ptr<KvConnClient> client, int timeout) {
    ...
    if (!status.ok()) {
      std::string err_msg = std::string(Trait::err_msg()) +
...
template <typename T> using RpcCallPtr = std::unique_ptr<RpcCall<T>>;
