...
struct RegionVerID {
  ...
  RegionVerID(uint64_t id_, uint64_t conf_ver_, uint64_t ver_)
      : id(id_), conf_ver(conf_ver_), ver(ver_) {}
  ...
  std::string toString() const {
    return "{" + std::to_string(id) + "," + std::to_string(conf_ver) + "," +
           std::to_string(ver) + "}";
  }
};
...
struct Region {
  ...
  Region(const metapb::Region &meta_, const metapb::Peer &peer_,
         const std::vector<metapb::Peer> &learners_)
  ...
  bool contains(const std::string &key) {
    return key >= startKey() && (key < endKey() || meta.end_key() == "");
  }
...
struct KeyLocation {
  ...
  KeyLocation() = default;
  KeyLocation(const RegionVerID &region_, const std::string &start_key_,
              const std::string &end_key_)
      : region(region_), start_key(start_key_), end_key(end_key_) {}

  bool contains(const std::string &key) {
    return key >= start_key && (key < end_key || end_key == "");
  }
};
...
struct RPCContext {
  ...
  RPCContext(const RegionVerID &region_, const metapb::Region &meta_,
             const metapb::Peer &peer_, const std::string &addr_)
...
public:
  RegionCache(pd::ClientPtr pdClient_, std::string key_, std::string value_)
      : pd_client(pdClient_), learner_key(std::move(key_)),
        learner_value(std::move(value_)), log(&Logger::get("pingcap.tikv")) {}

  RPCContextPtr getRPCContext(Backoffer &bo, const RegionVerID &id,
                              const StoreType store_type = TiKV);

  void updateLeader(Backoffer &bo, const RegionVerID &region_id,
  ...
  void onRegionStale(Backoffer &bo, RPCContextPtr ctx,
                     const errorpb::EpochNotMatch &epoch_not_match);
  ...
  std::pair<std::unordered_map<RegionVerID, std::vector<std::string>>,
            RegionVerID>
  groupKeysByRegion(Backoffer &bo, const std::vector<std::string> &keys);
...
private:
  ...
  std::vector<metapb::Peer> selectLearner(Backoffer &bo,
