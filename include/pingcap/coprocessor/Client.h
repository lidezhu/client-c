...
public:
  ...
  ResponseIter(Request *req_, std::vector<copTask> &&tasks_,
               kv::Cluster *cluster_, int concurrency_, Logger *log_)
      : cop_req(req_), tasks(std::move(tasks_)), cluster(cluster_),
        concurrency(concurrency_), cancelled(false), log(log_) {}
  ...
  std::pair<Result, bool> next() {
    ...
    cond_var.wait(lk, [this] {
      return unfinished_thread == 0 || cancelled || results.size() > 0;
    });
    if (cancelled) {
...
struct Client {
  static ResponseIter send(kv::Cluster *cluster, Request *cop_req,
                           int concurrency,
                           kv::StoreType store_type = kv::TiKV);
};
