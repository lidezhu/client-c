...
class TestWithMockKV
    : public testing::TestWithParam<std::tuple<char *, char *>> {
public:
  void SetUp() override {
...
TEST_P(TestWithMockKV, testGetInjectError) {
  ...
  mock_kv_cluster->updateFailPoint(mock_kv_cluster->stores[0].id, fail_point,
                                   fail_arg);
...
INSTANTIATE_TEST_SUITE_P(
    RunGetWithInjectedErr, TestWithMockKV,
    testing::Values(
        std::make_tuple<char *, char *>("server-is-busy", "2*return()"),
