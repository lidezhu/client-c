...
struct Store {
  int id;
  void deser(Poco::JSON::Object::Ptr json_obj) {
    id = json_obj->getValue<int64_t>("id");
  }
};
...
struct Cluster {
  ...
  void updateFailPoint(int store_id, std::string fail_point, std::string term) {
    ...
    HTTPRequest req(HTTPRequest::HTTP_POST,
                    std::string(mock_server) + "/mock-tikv/api/v1/clusters/" +
                        std::to_string(id) + "/stores/" +
                        std::to_string(store_id) + "/failpoints/" + fail_point);
...
inline ClusterPtr initCluster() {
  ...
  for (size_t i = 0; i < pd_arr->size(); i++) {
    urls.push_back(pd_arr->getObject(i)
                       ->getArray("client_urls")
                       ->getElement<std::string>(0));
  }
