...
std::vector<copTask>
buildCopTasks(kv::Backoffer &bo, kv::Cluster *cluster,
              std::vector<KeyRange> ranges, Request *cop_req, kv::StoreType,
              Logger *log = &Logger::get("pingcap/coprocessor"));
}
} // namespace pingcap
...
TEST_F(TestCoprocessor, testBuildTask1) {
  ...
  auto tasks = pingcap::coprocessor::buildCopTasks(
      bo, test_cluster.get(), ranges, nullptr, kv::StoreType::TiKV);
  ...
  pingcap::coprocessor::ResponseIter iter =
      pingcap::coprocessor::Client::send(test_cluster.get(), &req, 8);
  iter.open();
