  ...
  for (;;) {
    ...
    for (auto peer : peers) {
      ...
      if (addr.empty()) {
        ...
        bo.backoff(
            boRegionMiss,
            Exception("miss store, region id is: " + std::to_string(id.id) +
                          " store id is: " + std::to_string(peer.store_id()),
                      StoreNotReady));
        continue;
      }
    ...
    bo.backoff(boRegionMiss,
               Exception("region miss, region id is: " + std::to_string(id.id),
                         RegionUnavailable));
  }
}
...
std::vector<metapb::Peer>
RegionCache::selectLearner(Backoffer &bo, const metapb::Region &meta) {
  ...
  for (;;) {
    try {
      ...
      if (!meta.IsInitialized()) {
        throw Exception("region not found for regionID " +
      ...
      RegionPtr region = std::make_shared<Region>(meta, meta.peers(0),
                                                  selectLearner(bo, meta));
      if (leader.IsInitialized()) {
        region->switchPeer(leader.store_id());
      }
      log->debug("load region id: " + std::to_string(region->meta.id()) +
                 " leader store id " + std::to_string(leader.store_id()));
      return region;
...
RegionPtr RegionCache::loadRegionByKey(Backoffer &bo, const std::string &key) {
  for (;;) {
    try {
      ...
      if (!meta.IsInitialized()) {
        throw Exception("region not found for region key " + key,
                        RegionUnavailable);
      }
      if (meta.peers_size() == 0) {
        throw Exception("Receive Region with no peer", RegionUnavailable);
...
metapb::Store RegionCache::loadStore(Backoffer &bo, uint64_t id) {
  for (;;) {
    try {
      ...
      log->information("load store id " + std::to_string(id) + " address %s",
                       store.address());
      return store;
    } catch (Exception &e) {
      bo.backoff(boPDRPC, e);
...
Store RegionCache::reloadStore(Backoffer &bo, uint64_t id) {
  ...
  auto it = stores.emplace(
      id, Store(id, store.address(), store.peer_address(), labels));
...
RegionPtr RegionCache::searchCachedRegion(const std::string &key) {
  ...
  if (regions_map.begin() != regions_map.end() &&
      regions_map.begin()->second->contains(key)) {
...
void RegionCache::dropRegion(const RegionVerID &region_id) {
  ...
  if (iter_by_id != regions.end()) {
    ...
    log->information("drop region " + std::to_string(region_id.id) +
                     " because of send failure");
  }
}
...
void RegionCache::dropStore(uint64_t failed_store_id) {
  ...
  if (stores.erase(failed_store_id)) {
    log->information("drop store " + std::to_string(failed_store_id) +
                     " because of send failure");
  }
}
...
void RegionCache::updateLeader(Backoffer &bo, const RegionVerID &region_id,
...
void RegionCache::onRegionStale(Backoffer &bo, RPCContextPtr ctx,
                                const errorpb::EpochNotMatch &stale_epoch) {
  ...
  for (int i = 0; i < stale_epoch.current_regions_size(); i++) {
    ...
    RegionPtr region =
        std::make_shared<Region>(meta, meta.peers(0), selectLearner(bo, meta));
    region->switchPeer(ctx->peer.store_id());
    insertRegionToCache(region);
  }
}

std::pair<std::unordered_map<RegionVerID, std::vector<std::string>>,
          RegionVerID>
