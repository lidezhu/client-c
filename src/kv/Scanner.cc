...
void Scanner::resolveCurrentLock(pingcap::kv::Backoffer &bo,
...
void Scanner::getData(Backoffer &bo) {
  ...
  for (;;) {
    ...
    if (req_end_key.size() > 0 && loc.end_key.size() > 0 &&
        loc.end_key < req_end_key)
    ...
    if (pairs_size < batch) {
      ...
      // If the end key is empty, it infers this region is last and should stop
      // scan.
