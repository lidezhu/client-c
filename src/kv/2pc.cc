...
void TwoPhaseCommitter::prewriteSingleBatch(Backoffer &bo,
    ...
    if (response->errors_size() != 0) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (err.has_already_exist()) {
          throw Exception("key : " + err.already_exist().key() +
                              " has existed.",
                          LogicalError);
        }
...
void TwoPhaseCommitter::commitSingleBatch(Backoffer &bo,
                                          const BatchKeys &batch) {
...
void TTLManager::keepAlive(TwoPhaseCommitter &committer) {
  for (;;) {
    ...
    uint64_t uptime =
        extractPhysical(now) - extractPhysical(committer.start_ts);
    uint64_t new_ttl = uptime + managedLockTTL;
    std::ignore =
        sendTxnHeartBeat(bo, committer.cluster, committer.primary_lock,
                         committer.start_ts, new_ttl);
  }
}
