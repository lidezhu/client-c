...
void Cluster::splitRegion(const std::string &split_key) {
  ...
  region_cache->getRegionByID(bo,
                              RegionVerID(lr.id(), lr.region_epoch().conf_ver(),
                                          lr.region_epoch().version()));
  region_cache->getRegionByID(bo,
                              RegionVerID(rr.id(), rr.region_epoch().conf_ver(),
                                          rr.region_epoch().version()));
}
