...
int64_t LockResolver::ResolveLocks(Backoffer &bo, uint64_t caller_start_ts,
...
int64_t LockResolver::resolveLocks(Backoffer &bo, uint64_t caller_start_ts,
  ...
  for (auto &lock : locks) {
    ...
    if (status.ttl == 0) {
      ...
      auto before_txn_expired_time =
          cluster->oracle->untilExpired(lock->txn_id, status.ttl);
      before_txn_expired.update(before_txn_expired_time);
      if (for_write) {
        // Write conflict detected!
        // If it's a optimistic conflict and current txn is earlier than the
        // lock owner, abort current transaction. This could avoids the deadlock
        // scene of two large transaction.
        if (lock->lock_type != ::kvrpcpb::PessimisticLock &&
...
int64_t LockResolver::resolveLocksForWrite(Backoffer &bo,
...
TxnStatus LockResolver::getTxnStatus(Backoffer &bo, uint64_t txn_id,
  ...
  for (;;) {
    ...
    if (response->has_error()) {
      ...
      if (key_error.has_txn_not_found()) {
        ...
        throw Exception("unexpected err :" + key_error.ShortDebugString(),
...
void LockResolver::resolveLock(Backoffer &bo, LockPtr lock, TxnStatus &status,
                               std::unordered_set<RegionVerID> &set) {
  for (;;) {
    ...
    if (response->has_error()) {
      throw Exception("unexpected err :" + response->error().ShortDebugString(),
                      ErrorCodes::UnknownError);
    }
    if (lock->txn_size >= bigTxnThreshold) {
...
void LockResolver::resolvePessimisticLock(
    Backoffer &bo, LockPtr lock, std::unordered_set<RegionVerID> &set) {
  for (;;) {
    auto loc = cluster->region_cache->locateKey(bo, lock->key);
    if (set.count(loc.region) > 0) {
      return;
    ...
    if (!key_errors.empty()) {
      log->error("unexpected resolve pessimistic lock err: " +
                 key_errors[0].ShortDebugString());
      throw Exception("unexpected err :" + key_errors[0].ShortDebugString(),
                      ErrorCodes::UnknownError);
    }
    return;
  }
...
TxnStatus LockResolver::getTxnStatusFromLock(Backoffer &bo, LockPtr lock,
                                             uint64_t caller_start_ts) {
  uint64_t current_ts;
  if (lock->ttl == 0) {
  ...
  for (;;) {
    try {
      return getTxnStatus(bo, lock->txn_id, lock->primary, caller_start_ts,
                          current_ts, rollback_if_not_exists);
    } catch (Exception &e) {
    ...
    auto before_txn_expired_time =
        cluster->oracle->untilExpired(lock->txn_id, lock->ttl);
    if (before_txn_expired_time <= 0) {
      log->warning("lock txn not found, lock has expired. " +
                   getLockInfo(lock));
      if (lock->lock_type == ::kvrpcpb::PessimisticLock) {
        return TxnStatus{};
      }
