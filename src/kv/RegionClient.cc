...
void RegionClient::onRegionError(Backoffer &bo, RPCContextPtr rpc_ctx,
                                 const errorpb::Error &err) {
  if (err.has_not_leader()) {
    auto not_leader = err.not_leader();
    if (not_leader.has_leader()) {
      log->information("not leader but has leader, result store is %d",
                       (int)not_leader.leader().store_id());
      cluster->region_cache->updateLeader(bo, rpc_ctx->region,
                                          not_leader.leader().store_id());
...
void RegionClient::onSendFail(Backoffer &bo, const Exception &e,
                              RPCContextPtr rpc_ctx) {
  cluster->region_cache->onSendReqFail(rpc_ctx, e);
  // Retry on send request failure when it's not canceled.
  // When a store is not available, the leader of related region should be
  // elected quickly.
