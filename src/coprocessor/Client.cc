...
std::vector<copTask> buildCopTasks(kv::Backoffer &bo, kv::Cluster *cluster,
                                   std::vector<KeyRange> ranges,
                                   Request *cop_req, kv::StoreType store_type,
                                   Logger *log) {
...
ResponseIter Client::send(kv::Cluster *cluster, Request *cop_req,
                          int concurrency, kv::StoreType store_type) {
  ...
  auto tasks =
      buildCopTasks(bo, cluster, cop_req->ranges, cop_req, store_type, log);
  return ResponseIter(cop_req, std::move(tasks), cluster, concurrency, log);
}

std::vector<copTask> ResponseIter::handle_task_impl(kv::Backoffer &bo,
                                                    const copTask &task) {
  ...
  if (resp->has_locked()) {
    ...
    auto before_expired = cluster->lock_resolver->ResolveLocks(
        bo, task.req->start_ts, locks, pushed);
    if (!pushed.empty()) {
      cluster->min_commit_ts_pushed.insert(pushed.begin(), pushed.end());
    }
    if (before_expired > 0) {
      bo.backoffWithMaxSleep(
          kv::boTxnLockFast, before_expired,
          Exception(resp->locked().DebugString(), ErrorCodes::LockError));
    }
    return buildCopTasks(bo, cluster, task.ranges, cop_req, task.store_type,
                         log);
  }
  ...
  if (!err_msg.empty()) {
    throw Exception("Coprocessor error: " + err_msg,
                    ErrorCodes::CoprocessorError);
  }
...
void ResponseIter::handle_task(const copTask &task) {
  ...
  while (idx < remain_tasks.size()) {
    try {
      ...
      if (new_tasks.size() > 0) {
        remain_tasks.insert(remain_tasks.end(), new_tasks.begin(),
                            new_tasks.end());
      }
