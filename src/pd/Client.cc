...
addrsToUrls(const std::vector<std::string> &addrs) {
...
Client::Client(const std::vector<std::string> &addrs)
    : max_init_cluster_retries(100), pd_timeout(3), loop_interval(100),
      update_leader_interval(60), urls(addrsToUrls(addrs)),
...
std::shared_ptr<Client::PDConnClient>
Client::getOrCreateGRPCConn(const std::string &addr) {
  ...
  // TODO Check Auth
  Poco::URI uri(addr);
  auto client_ptr = std::make_shared<PDConnClient>(
...
pdpb::GetMembersResponse Client::getMembers(std::string url) {
  ...
  auto status =
      client->stub->GetMembers(&context, pdpb::GetMembersRequest{}, &resp);
  if (!status.ok()) {
    std::string err_msg =
        "get member failed: " + std::to_string(status.error_code()) + ": " +
...
void Client::switchLeader(
    const ::google::protobuf::RepeatedPtrField<std::string> &leader_urls) {
...
void Client::leaderLoop() {
  ...
  for (;;) {
    ...
    if (update_leader_cv.wait_until(lk, now + loop_interval,
                                    [this]() { return check_leader.load(); })) {
      should_update = true;
    } else {
      ...
      if (std::chrono::system_clock::now() >= next_update_time) {
        should_update = true;
        next_update_time =
            std::chrono::system_clock::now() + update_leader_interval;
      }
    }
    if (should_update) {
      try {
...
uint64_t Client::getGCSafePoint() {
  ...
  auto status =
      leaderClient()->stub->GetGCSafePoint(&context, request, &response);
  if (!status.ok()) {
    err_msg = "get safe point failed: " + std::to_string(status.error_code()) +
              ": " + status.error_message();
    log->error(err_msg);
    check_leader.store(true);
...
Client::getRegionByKey(const std::string &key) {
  ...
  if (!status.ok()) {
    std::string err_msg =
        ("get region failed: " + std::to_string(status.error_code()) + " : " +
         status.error_message());
    log->error(err_msg);
    check_leader.store(true);
    throw Exception(err_msg, GRPCErrorCode);
...
std::pair<metapb::Region, metapb::Peer>
Client::getRegionByID(uint64_t region_id) {
  ...
  auto status =
      leaderClient()->stub->GetRegionByID(&context, request, &response);
  if (!status.ok()) {
    std::string err_msg =
        ("get region by id failed: " + std::to_string(status.error_code()) +
         ": " + status.error_message());
    log->error(err_msg);
    check_leader.store(true);
    throw Exception(err_msg, GRPCErrorCode);
...
metapb::Store Client::getStore(uint64_t store_id) {
  ...
  if (!status.ok()) {
    std::string err_msg =
        ("get store failed: " + std::to_string(status.error_code()) + ": " +
         status.error_message());
    log->error(err_msg);
    check_leader.store(true);
    throw Exception(err_msg, GRPCErrorCode);
